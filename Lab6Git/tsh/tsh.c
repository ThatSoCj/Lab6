...
void OpGet()
{
   ...
   /* skip deletion so GET does not consume the tuple */
...
void OpShell() {
    ...
    int           pfd[2];
    pid_t         pid;
    char          outbuf[MAX_SHELL_OUT];
    int           nbytes;

    // 1) Read the incoming request
    if (!readn(newsock, (char*)&req, sizeof(req))) return;
    cmdlen = ntohl(req.length);
    cmd = malloc(cmdlen + 1);
    if (!readn(newsock, cmd, cmdlen)) { free(cmd); return; }
    cmd[cmdlen] = '\0';

    // 2) Fork & exec the shell command
    if (pipe(pfd) < 0) { perror("pipe"); free(cmd); return; }
    if ((pid = fork()) < 0) { perror("fork"); close(pfd[0]); close(pfd[1]); free(cmd); return; }
    if (pid == 0) {
        close(pfd[0]);
    ...
    close(pfd[1]);
    nbytes = read(pfd[0], outbuf, sizeof(outbuf)-1);
    if (nbytes < 0) nbytes = 0;
    outbuf[nbytes] = '\0';
    close(pfd[0]);
    waitpid(pid, NULL, 0);

    // 3) Send back a success response with the captured output
    resp.status = htons(SUCCESS);
    resp.error  = htons(TSH_ER_NOERROR);
    strncpy(resp.stdout, outbuf, sizeof(resp.stdout));
    writen(newsock, (char*)&resp, sizeof(resp));
